...
let state = {
  alarm1FiredAt: null, alarm2FiredAt: null, escalationStartedAt: null,
  responseReceived: false, timeLow: 0, lastGlucose: null, lastTrend: null,
  contacts: [], pushSubscriptions: [], libreToken: null, libreTokenExpiry: null,
};
...
function loadSettings() {
  ...
  } catch (e) { console.log('No settings file yet, using defaults'); }
}

function saveSettings() {
  fs.writeFileSync(SETTINGS_FILE, JSON.stringify({
    contacts: state.contacts, pushSubscriptions: state.pushSubscriptions,
    threshold: CONFIG.GLUCOSE_THRESHOLD, lowDuration: CONFIG.LOW_DURATION_MINS,
    alarmGap: CONFIG.ALARM_GAP_MINS, waWindow: CONFIG.WA_WINDOW_MINS,
...
function setupWebPush() {
  if (CONFIG.VAPID_PUBLIC && CONFIG.VAPID_PRIVATE) {
    try {
      ...
      console.warn('⚠️  VAPID key error — push notifications disabled:', e.message);
    }
  } else {
...
async function libreLogin() {
  ...
  try {
    ...
    console.error('LibreLinkUp login failed:', data?.error || 'unknown');
    return false;
  } catch (e) { console.error('LibreLinkUp login error:', e.message); return false; }
}
...
async function getGlucoseReading() {
  ...
  try {
    ...
    return { glucose: reading.Value, trend: trendMap[reading.TrendArrow] || 'stable', timestamp: reading.FactoryTimestamp };
  } catch (e) { console.error('Error fetching glucose:', e.message); return null; }
}
...
async function sendWhatsAppToTop3() {
  ...
  for (const contact of top3) {
    try {
      await client.messages.create({ from: CONFIG.TWILIO_WHATSAPP_FROM, to: `whatsapp:${contact.phone}`, body: WA_MESSAGE });
      console.log(`✅ WhatsApp sent to ${contact.name}`);
    } catch (e) { console.error(`WhatsApp to ${contact.name} failed:`, e.message); }
  }
}
...
async function callContact(contact, attempt) {
  ...
  try {
    const twiml = `<Response><Say voice="alice" language="es-ES">Hola. Este es un mensaje automático. La persona a quien intentas contactar tiene el azúcar muy baja y puede estar inconsciente. Por favor llámala inmediatamente.</Say></Response>`;
    ...
    return true;
  } catch (e) { console.error(`Call to ${contact.name} failed:`, e.message); return false; }
}
...
async function checkGlucoseAndAlarm() {
  ...
  const shouldFireAlarm1 = !state.alarm1FiredAt && (lowLongEnough || (glucose < CONFIG.GLUCOSE_THRESHOLD && isFallingFast && state.timeLow >= 5));
  if (shouldFireAlarm1) {
    state.alarm1FiredAt = Date.now();
    console.log('🔔 ALARM 1 firing');
    await sendPushNotification('🔔 Low glucose — eat something now', `Glucose is ${glucose} mg/dL. Drink juice or take glucose tablets.`, { type: 'alarm1', glucose, trend });
...
async function checkAlarm2() {
  ...
  console.log('⚠️ ALARM 2 firing');
  await sendPushNotification('⚠️ Glucose still low — open app now', `Glucose is ${glucose} mg/dL. Consciousness check required.`, { type: 'alarm2', glucose, trend, urgent: true });
  setTimeout(async () => {
    if (!state.responseReceived && state.alarm2FiredAt) {
      console.log('⚠️ No response — auto-escalating');
...
function resetAlarmState() {
  state.alarm1FiredAt = null; state.alarm2FiredAt = null;
  state.escalationStartedAt = null; state.responseReceived = false; state.timeLow = 0;
}

function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

app.get('/api/health', (req, res) => {
  res.json({ ok: true, lastGlucose: state.lastGlucose, lastTrend: state.lastTrend, timeLow: state.timeLow, alarm1Active: !!state.alarm1FiredAt, alarm2Active: !!state.alarm2FiredAt, escalationActive: !!state.escalationStartedAt, responseReceived: state.responseReceived, contactCount: state.contacts.length, pushSubscriptions: state.pushSubscriptions.length });
});
...
app.post('/api/alarm/conscious', (req, res) => {
  ...
  state.responseReceived = true; resetAlarmState();
  res.json({ ok: true });
});
...
app.get('/api/settings', (req, res) => {
  res.json({ contacts: state.contacts, threshold: CONFIG.GLUCOSE_THRESHOLD, lowDuration: CONFIG.LOW_DURATION_MINS, alarmGap: CONFIG.ALARM_GAP_MINS, waWindow: CONFIG.WA_WINDOW_MINS, callAttempts: CONFIG.CALL_ATTEMPTS });
});
